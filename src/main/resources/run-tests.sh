...
if [ "$1" == "" ]; then
	...
	cd ../../../
	mvn clean verify -Denvironment=$ENVIRONMENT -Dbrowser=$BROWSER -Ddriver.remote.url=$DRIVER_REMOTE_URL -Dtimeout=$TIMEOUT
  exit
fi
...
if [ "$1" != "" ]; then
	...
	cd ../../../
	mvn clean install -Dcucumber.options="--tags @$1" -Denvironment=$ENVIRONMENT -Dbrowser=$BROWSER -Ddriver.remote.url=$DRIVER_REMOTE_URL -Dtimeout=$TIMEOUT
	exit
fi
